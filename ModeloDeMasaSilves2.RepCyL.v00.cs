...
namespace EngineTest
{
  public class MassModelTemplate : MassModelBase
  {
	...
	parA    = 40.3331;
	parB	= 1.5003;
	plot.SI = parA / (1 - (1 - parA / plot.H_DOMINANTE.Value) * Math.Pow( (double) plot.EDAD.Value / 50, parB ) );
	// VOLUMEN
	...
	parA    = 40.3331;
	parB	= 1.5003;
	newPlot.H_DOMINANTE = parA / (1 - (1 - parA / newPlot.SI.Value) * Math.Pow( (double) newPlot.EDAD.Value / 50, -1*parB ) );
	// AREA_BASIMETRICA:
